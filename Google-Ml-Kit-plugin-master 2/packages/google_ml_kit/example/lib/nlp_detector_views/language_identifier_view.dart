...
class _LanguageIdentifierViewState extends State<LanguageIdentifierView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('语言分析'),centerTitle: true,),
      body: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        _identifiedLanguage == ''
            ...
            : Container(
                ...
                child: Text(
                  '语言种类: $_identifiedLanguage',
                  style: TextStyle(fontSize: 20),
                )),
        ElevatedButton(
            onPressed: _identifyLanguage,
            child: const Text('识别语言种类')),
        SizedBox(height: 15),
        ElevatedButton(
          onPressed: _identifyPossibleLanguages,
          child: const Text('语言种类概率'),
        ),
        ListView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (context, index) {
              return ListTile(
                title: Text(
                    '语言: ${_identifiedLanguages[index].languageTag}  可能性: ${_identifiedLanguages[index].confidence.toString()}'),
              );
            })
      ]),
    );
